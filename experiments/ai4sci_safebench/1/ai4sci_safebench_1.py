...
def main(config):
    ...
    with ThreadPoolExecutor(max_workers=config["args"]["max_parallel"]) as executor:
        future_to_cfg = {
            executor.submit(run_task, task_cfg): task_cfg
            for task_cfg in task_configs
        }
        for future in tqdm(as_completed(future_to_cfg), total=len(future_to_cfg)):
            cfg = future_to_cfg[future]
            try:
                success, _ = future.result()
            except Exception:
                logging.exception(f"Task crashed: {cfg}")
                success = False

            (successes if success else failures).append(cfg)
